...
from billing_payment.alipay.exceptions import MissingParameter
from billing_payment.alipay.exceptions import ParameterValueError
from billing_payment.alipay.exceptions import TokenAuthorizationError
from billing_payment.alipay.conf import *
...
class Alipay(object):
    ...
    def request_url(self,**kwargs):
        ...
        if check_method!=None:
            check_method(**self.params)
            return self._build_url()

    #根据参数请求不同的支付宝服务，在请求时候更新self.params,添加如 订单号，价格，物流等信息
    def request(self,request_method,**kwargs):
        self.params.update(kwargs)
        service=self.params.get('service')
        if request_method=='get':
            ...
            if check_method!=None:
                check_method(**self.params)
                post_info=self._build_sign_params(**self.params)
                data=requests.post(self.GATEWAY_URL,data=post_info)
